...
(function () {
   ...
   */
  function initHomeInfoFilter() {
    var host = document.getElementById('home-info-filter');
    if (!host) return;
    ...
    function applyVisibility() {
      SECTIONS.forEach(function (s) {
        ...
        if (el) el.style.display = selected.has(s.key) ? '' : 'none';
      });
    }

    function rerender() {
      host.querySelectorAll('.home-info-filter-btn').forEach(function (b) {
        var on = selected.has(b.dataset.infoKey);
        b.classList.toggle('is-on', on);
        b.setAttribute('aria-pressed', on ? 'true' : 'false');
      });
      if (statusEl) statusEl.textContent = selectedSummary();
      applyVisibility();
      try {
        localStorage.setItem(storageKey, JSON.stringify(Array.from(selected)));
      } catch (e) {}
    ...
    function previewLabel(key) {
      if (statusEl)
        statusEl.textContent = (selected.has(key) ? 'Selected: ' : 'Filter: ') + labelForKey(key);
    }

    host.addEventListener('mouseover', function (e) {
      var b = e.target && e.target.closest ? e.target.closest('.home-info-filter-btn') : null;
      if (!b || !b.dataset.infoKey) return;
      previewLabel(b.dataset.infoKey);
    });
